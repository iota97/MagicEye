...
The dual quaternion computation does not use neither \textbf{glm::quat} or \textbf{glm::dualquat}, the first because the multiplication returns a normalized 
result that is fine when using quaterion for rotation but does not work for dual quaterion operation; the second one because the class is completely lacking
of any useful methods needed by the application.
\begin{center}
    \centering
